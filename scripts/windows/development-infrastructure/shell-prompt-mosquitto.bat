...
rem ============================================================================
rem shell-prompt-mosquitto.bat
rem
rem Open an interactive shell inside the Mosquitto container used by
rem development-infrastructure mode.
rem
rem The script:
...
rem   - opens an interactive shell in the running diaries-mqtt service.
rem
rem Typical commands after entering the container include:
rem
rem     ls -l /mosquitto/config
rem     cat /mosquitto/config/mosquitto.conf
rem     cat /mosquitto/config/aclfile.txt
rem     cat /mosquitto/config/pwfile.txt
rem ============================================================================
...
rem ----------------------------------------------------------------------------
rem Define and validate the Compose file for development-infrastructure mode.
rem ----------------------------------------------------------------------------
...
rem local.env take precedence over values in development-infrastructure.env.
rem ----------------------------------------------------------------------------
...
rem ----------------------------------------------------------------------------
rem Open an interactive shell in the running Mosquitto container.
rem ----------------------------------------------------------------------------

echo Opening shell in the Diaries development-infrastructure Mosquitto container...
...
docker compose ^
    ...
    --env-file "%ENV_FILE%" ^
    --env-file "%LOCAL_ENV_FILE%" ^
    exec diaries-mqtt sh

set "EXIT_CODE=%ERRORLEVEL%"

if not "%EXIT_CODE%"=="0" (
    echo ERROR: Failed to open a shell in the Mosquitto container. >&2
    goto :cleanup
)
