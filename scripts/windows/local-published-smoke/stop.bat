...
rem stop.bat
rem
rem Stop and remove the Diaries local-published-smoke Compose stack.
rem ============================================================================
...
rem Stop the stack.
rem
rem Both environment files are supplied so Compose resolves the same project
rem configuration that was used when the stack was started.
rem ----------------------------------------------------------------------------
...
if not "%EXIT_CODE%"=="0" (
    echo ERROR: Failed to stop the Diaries local-published-smoke stack. >&2
    goto :cleanup
)

echo Diaries local-published-smoke stack stopped successfully.
