...
rem ============================================================================
rem shell-prompt-responder.bat
rem
rem Open an interactive shell inside the Responder container used by
...
rem   - opens an interactive shell in the running diaries-responder service.
rem
rem Typical commands after entering the container include:
rem
rem     ps
rem     ls -l /opt/diaries
rem     env | sort
rem ============================================================================
...
rem ----------------------------------------------------------------------------
rem Open an interactive shell in the running Responder container.
rem ----------------------------------------------------------------------------

echo Opening shell in the Diaries local-published-smoke Responder container...
...
docker compose ^
    ...
    --env-file "%ENV_FILE%" ^
    --env-file "%LOCAL_ENV_FILE%" ^
    exec diaries-responder sh

set "EXIT_CODE=%ERRORLEVEL%"

if not "%EXIT_CODE%"=="0" (
    echo ERROR: Failed to open a shell in the Responder container. >&2
    goto :cleanup
)
