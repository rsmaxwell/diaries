...
rem start.bat
rem
rem Start the Diaries application in local-published-smoke mode.
rem
rem The script:
rem   - locates the Diaries project root;
rem   - validates and loads the mode and machine-local environment files;
rem   - applies optional local image-tag overrides;
rem   - validates the resolved Compose configuration;
rem   - pulls the selected published images;
rem   - starts the complete smoke-test stack and waits for it to become healthy.
rem ============================================================================
...
rem ----------------------------------------------------------------------------
rem Report the published images that will be used.
rem ----------------------------------------------------------------------------

echo Starting Diaries local-published-smoke mode.
...
rem ----------------------------------------------------------------------------
rem Pull the selected published images.
rem ----------------------------------------------------------------------------
...
rem ----------------------------------------------------------------------------
rem Start the stack and wait for its services to become healthy.
rem
rem The Compose project name is deliberately not supplied here. It is defined
rem by the top-level "name:" property in compose.local-published-smoke.yaml.
rem ----------------------------------------------------------------------------
...
if not "%EXIT_CODE%"=="0" (
    echo ERROR: Failed to start the Diaries local-published-smoke stack. >&2
    goto :cleanup
)

echo.
echo Diaries local-published-smoke stack started successfully.
