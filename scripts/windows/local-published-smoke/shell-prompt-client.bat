...
rem ============================================================================
rem shell-prompt-client.bat
rem
rem Open an interactive shell inside the Client container used by
...
rem   - opens an interactive shell in the running diaries-client service.
rem
rem Typical commands after entering the container include:
rem
rem     ps
rem     ls -l /usr/share/nginx/html
rem     cat /etc/nginx/nginx.conf
rem ============================================================================
...
rem ----------------------------------------------------------------------------
rem Open an interactive shell in the running Client container.
rem ----------------------------------------------------------------------------

echo Opening shell in the Diaries local-published-smoke Client container...
...
docker compose ^
    ...
    --env-file "%ENV_FILE%" ^
    --env-file "%LOCAL_ENV_FILE%" ^
    exec diaries-client sh

set "EXIT_CODE=%ERRORLEVEL%"

if not "%EXIT_CODE%"=="0" (
    echo ERROR: Failed to open a shell in the Client container. >&2
    goto :cleanup
)
