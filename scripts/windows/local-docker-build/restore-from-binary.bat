...
rem ============================================================================
rem restore-from-binary.bat
rem
rem Replace the local-docker-build PostgreSQL database from a PostgreSQL custom
rem format dump created by pg_dump --format=custom.
rem
rem Usage:
rem
rem     restore-from-binary.bat [dump-file]
rem
...
rem local-docker-build .dump file is selected from:
rem
...
rem on the standard local-docker-build backup directory existing.
rem
...
if not "%~1"=="" (
    ...
    for /f "delims=" %%F in ('dir /b /a-d /o-d "%BACKUP_DIR%\diaries-local-docker-build-*.dump" 2^>nul') do (
...
rem ----------------------------------------------------------------------------
rem Check that the local-docker-build PostgreSQL service is available.
rem ----------------------------------------------------------------------------
...
if errorlevel 1 (
    echo ERROR: The local-docker-build database is not ready. >&2
...
echo.
echo WARNING: This will completely replace the local-docker-build database.
echo.
