...
rem ============================================================================
rem restore-from-sql.bat
rem
rem Replace the local-docker-build PostgreSQL database from a plain-text SQL
rem dump created by pg_dump --format=plain.
rem
rem Usage:
rem
rem     restore-from-sql.bat [sql-file]
rem
...
rem local-docker-build .sql file is selected from:
rem
...
if not "%~1"=="" (
    ...
    for /f "delims=" %%F in ('dir /b /a-d /o-d "%BACKUP_DIR%\diaries-local-docker-build-*.sql" 2^>nul') do (
