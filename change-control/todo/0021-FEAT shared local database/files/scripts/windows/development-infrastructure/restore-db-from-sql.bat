...
rem Replace the PostgreSQL development database from a plain SQL dump.
rem
rem Usage:
...
if not exist "%BACKUP_FILE%" (
    echo ERROR: SQL backup file not found. >&2
...
echo.
echo WARNING: This will completely delete and recreate the development database.
echo All current data in "%LEDGER_DB_NAME%" will be replaced by the SQL backup.
echo.
echo IMPORTANT: Stop ledger-server before continuing so that it does not
echo reconnect to or recreate objects while the restore is taking place.
echo.
echo Backup:
...
if errorlevel 1 (
    echo ERROR: Could not terminate existing database connections. >&2
    set "EXIT_CODE=1"
    goto :cleanup
)
...
echo.
echo Restoring the Ledger development database from SQL...
