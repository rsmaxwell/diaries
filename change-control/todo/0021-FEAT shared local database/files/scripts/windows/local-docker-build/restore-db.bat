...
rem Restore the PostgreSQL database used by the full Docker environment.
rem Usage:
rem   restore-db.bat backups\ledger-YYYYMMDD-HHMMSS.dump
rem
rem This script restores a custom-format pg_dump file created by backup-db.bat.
rem It drops/recreates objects in the target database using pg_restore --clean --if-exists.

if "%~1"=="" (
    echo Usage: %~nx0 ^<backup-file.dump^>
    exit /b 1
)
...
@echo on
type "%BACKUP_FILE%" | docker compose -f "%COMPOSE_FILE%" --env-file "%ENV_FILE%" --env-file "%LOCAL_ENV_FILE%" exec -T "%LEDGER_DB_SERVICE%" pg_restore -U "%LEDGER_DB_USERNAME%" -d "%LEDGER_DB_NAME%" --clean --if-exists --no-owner
@echo off
...
echo Restart ledger-server so the application reconnects cleanly.
