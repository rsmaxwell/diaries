...
rem Reset the PostgreSQL database used by local-published-smoke mode.
rem
rem All local modes may share LEDGER_DB_DATA_DIR through local.env. Therefore
...
if not defined LEDGER_DB_DATA_DIR (
    echo ERROR: LEDGER_DB_DATA_DIR is not set. >&2
    set "EXIT_CODE=1"
    goto :cleanup
)

for %%I in ("%LEDGER_DB_DATA_DIR%") do set "DB_DATA_DIR=%%~fI"

echo.
echo This will:
echo   1. Stop the local published-image smoke-test stack.
...
echo          three local Ledger modes will be deleted.
echo.
set "ANSWER="
...
echo.
echo Ledger database reset successfully.
